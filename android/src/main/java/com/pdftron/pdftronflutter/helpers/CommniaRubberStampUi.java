...
public final class CommniaRubberStampUi {
    ...
    /** {@link RubberStampCreate#getCreateAnnotType()}. */
    ...
    public static void saveDefaultApprovedStampPreset(@NonNull Context context) {
        ...
        try {
            AnnotStyle style = ToolStyleConfig.getInstance()
                    .getAnnotPresetStyle(context, RUBBER_STAMP_ANNOT_TYPE, 0, "");
            if (style == null) {
                return;
            }
            style.setStampId(stampId);
            PdfViewCtrlSettingsManager.setAnnotStylePreset(
                    context, RUBBER_STAMP_ANNOT_TYPE, 0, "", style.toJSONString());
    ...
    private static void applyIfRubberStamp(@NonNull ToolManager toolManager, ToolManager.Tool tool) {
        ...
        applyDefaultCommniaApprovedStamp(rubberStamp, toolManager);
    }
    ...
    private static void applyDefaultCommniaApprovedStamp(
        ...
        Runnable apply = () -> {
            String currentPresetId = getSavedRubberStampPresetId(context);
            if (!shouldReplaceWithCommniaApproved(currentPresetId, context, commniaStampId)) {
                return;
            }
            tool.setStampName(commniaStampId);
            updatePresetBarStamp(toolManager, commniaStampId);
        };
        pdfViewCtrl.post(apply);
        pdfViewCtrl.postDelayed(apply, 150);
    }

    @Nullable
    private static String getSavedRubberStampPresetId(@NonNull Context context) {
        try {
            AnnotStyle style = ToolStyleConfig.getInstance()
                    .getAnnotPresetStyle(context, RUBBER_STAMP_ANNOT_TYPE, 0, "");
    ...
    private static void updatePresetBarStamp(
        ...
        PresetBarViewModel viewModel = findPresetBarViewModel(
